...
local antilagQueue = {}                                      -- instâncias novas esperando processamento
local antilagQueued = setmetatable({}, { __mode = "k" })     -- evita duplicar instância na fila

-- muda uma propriedade guardando o valor original
-- (sem pcall: essas classes sempre têm essas propriedades)
local function antilagSet(inst, prop, value)
	...
	if saved[prop] == nil then
		saved[prop] = inst[prop]
	end
	inst[prop] = value
end

-- só toca na propriedade se o valor for diferente (menos update de render à toa)
local function antilagSetIf(inst, prop, value)
	if inst[prop] ~= value then
		antilagSet(inst, prop, value)
	end
end
...
local function antilagProcess(inst)
	...
	if inst:IsA("BasePart") then
		antilagSetIf(inst, "Material", CONFIG.ANTILAG_MATERIAL)
		antilagSetIf(inst, "Reflectance", 0)
		antilagSetIf(inst, "CastShadow", false)
		if inst:IsA("MeshPart") then
			antilagSetIf(inst, "TextureID", "")
			antilagSetIf(inst, "RenderFidelity", Enum.RenderFidelity.Performance)
		end

	elseif inst:IsA("Decal") or inst:IsA("Texture") then
		antilagSetIf(inst, "Transparency", 1)

	elseif inst:IsA("SurfaceAppearance") then
		antilagSetIf(inst, "ColorMap", "")
		antilagSetIf(inst, "NormalMap", "")
		antilagSetIf(inst, "MetalnessMap", "")
		antilagSetIf(inst, "RoughnessMap", "")

	elseif inst:IsA("SpecialMesh") then
		antilagSetIf(inst, "TextureId", "")

	elseif inst:IsA("Shirt") then
		antilagSetIf(inst, "ShirtTemplate", "")
	elseif inst:IsA("Pants") then
		antilagSetIf(inst, "PantsTemplate", "")
	elseif inst:IsA("ShirtGraphic") then
		antilagSetIf(inst, "Graphic", "")

	elseif inst:IsA("ParticleEmitter") then
		if CONFIG.ANTILAG_PARTICLE_KEEP <= 0 then
			antilagSetIf(inst, "Enabled", false)
		else
			antilagSetIf(inst, "Rate", inst.Rate * CONFIG.ANTILAG_PARTICLE_KEEP)
		end
	...
	elseif inst:IsA("Explosion") then
		antilagSetIf(inst, "Visible", false)
	...
	elseif inst:IsA("Atmosphere") then
		antilagSetIf(inst, "Density", 0)
		antilagSetIf(inst, "Haze", 0)
		antilagSetIf(inst, "Glare", 0)
	end
end

local function antilagGlobal()
	antilagSetIf(Lighting, "GlobalShadows", false)

	local terrain = workspace:FindFirstChildOfClass("Terrain")
	if terrain then
		antilagSetIf(terrain, "WaterWaveSize", 0)
		antilagSetIf(terrain, "WaterWaveSpeed", 0)
		antilagSetIf(terrain, "WaterReflectance", 0)
		antilagSetIf(terrain, "Decoration", false)   -- grama/decalque do terrain pesa bastante
	end
...
function AntiLag.Enable()
	...
	-- instância nova entra na fila (semspawnar coroutine por evento)
	local function enqueue(inst)
		if not antilagQueued[inst] then
			antilagQueued[inst] = true
			table.insert(antilagQueue, inst)
		end
	end
	table.insert(antilagConns, workspace.DescendantAdded:Connect(enqueue))
	table.insert(antilagConns, Lighting.DescendantAdded:Connect(enqueue))

	-- mapa inicial + fila processados aos poucos num loop só
	task.spawn(function()
		local pending = workspace:GetDescendants()
		local i, n = 0, #pending
		while AntiLag.Enabled and run == antilagRun do
			local budget = CONFIG.ANTILAG_BATCH

			while budget > 0 and i < n do
				i += 1
				antilagProcess(pending[i])
				budget -= 1
			end
			if i >= n then
				pending = nil
			...
			if #antilagQueue > 0 then
				local batch = antilagQueue
				antilagQueue = {}
				for _, inst in ipairs(batch) do
					antilagQueued[inst] = nil
					antilagProcess(inst)
				end
			end
...
panel.Size = UDim2.fromOffset(280, 508)
...
list.Size = UDim2.new(1, -24, 0, 412)
...
createToggle("Anti Lag", 9, AntiLag.Set)

--// PAINEL ESP: Saint + Fragment + DogBane + Rokakaka + Players + Horse Speed
...
local CONFIG = {
	...
	HORSE_MAX_SPEED = 150,                           -- velocidade máxima
	HORSE_ACCEL = 25,                                -- aceleração (studs/s a cada segundo)
...
----------------------------------------------------------------
--// FUNÇÃO 6: HORSE SPEED (em cima do cavalo: acelera aos poucos até o máximo)
----------------------------------------------------------------
...
local function horseStep(dt)
	...
	-- acelera aos poucos até o máximo
	if horseState.model then
		for _, t in ipairs(horseState.targets) do
			if t.inst.Parent then
				if t.cur < CONFIG.HORSE_MAX_SPEED then
					t.cur = math.min(t.cur + CONFIG.HORSE_ACCEL * dt, CONFIG.HORSE_MAX_SPEED)
				end
				t.inst[t.prop] = t.cur
			end
		end
...
list.Size = UDim2.new(1, -24, 0, 268)
